...
enum AlertAuthorityProvider {
    static func authority(for countryName: String?) -> String {
        guard let country = countryName?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased(), !country.isEmpty else {
            return "National Meteorological & Civil Protection Service"
        }

        if country.contains("italy") || country.contains("italia") {
            return "Dipartimento della Protezione Civile / Servizio Meteorologico"
...
struct WeatherAlert: Identifiable, Sendable, Hashable {
    enum Severity: String, Sendable, Hashable {
        ...
        var labelText: String {
            ...
            case .advisory: return "Yellow advisory"
    ...
    static func detectAlerts(from weather: WeatherResponse?, useMetric: Bool = true) -> [WeatherAlert] {
        ...
        let todayTitle = today?.date ?? "today"

        // 1. High Heat Alert (Heatwave)
        if maxTempC >= 33.0 || feelsLikeC >= 35.0 {
            ...
            let sevText = isExtreme ? "Orange heatwave warning" : "Yellow high temperature advisory"
            ...
            alerts.append(WeatherAlert(
                ...
                title: sevText,
                description: "Expected maximum temperatures up to \(maxTDisplay) with high bioclimatic stress. Avoid prolonged exposure during peak hours and stay well hydrated.",
                source: sourceTitle,
                timeWindow: "Conditions expected throughout the day"
            ))
        }

        // 2. Extreme Cold & Freeze Alert
        if minTempC <= 0.0 || feelsLikeC <= -2.0 {
            ...
            let severity: Severity = isExtreme ? .warning : .advisory
            let sevText = isExtreme ? "Orange intense freeze warning" : "Yellow freeze & frost advisory"
            ...
            alerts.append(WeatherAlert(
                ...
                headline: "Severe weather",
                title: sevText,
                description: "Expected minimum temperatures down to \(minTDisplay) with frost and icing risk. Exercise caution on roads.",
                source: sourceTitle,
                timeWindow: "Conditions expected overnight and early morning"
            ))
        }
        ...
        if maxWindKmph >= 45.0 {
            ...
            let sevText = isGale ? "Orange severe gale warning" : "Yellow strong wind advisory"
            ...
            alerts.append(WeatherAlert(
                ...
                headline: "Severe weather",
                title: sevText,
                description: "Expected wind gusts up to \(windDisplay) with possible travel disruption.",
                source: sourceTitle,
                timeWindow: "Conditions expected in the coming hours"
            ))
        }

        // 4. Severe Storm Alert
        ...
        if stormCodes.contains(weatherCode) || (maxRainChance >= 75 && (weatherCode == 299 || weatherCode == 302 || weatherCode == 305 || weatherCode == 308)) {
            ...
            let sevText = isHeavyStorm ? "Orange severe storm warning" : "Yellow storm risk advisory"
            ...
            alerts.append(WeatherAlert(
                ...
                headline: "Severe weather",
                title: sevText,
                description: "Thunderstorms expected with possible lightning, localized hail and heavy rain showers.",
                source: sourceTitle,
                timeWindow: "Conditions expected until event concludes"
            ))
        }
